...
## Deploying on Coolify (same repo as the ticket bot)

1. **New Resource** → **Public Repository** → same repo URL, same branch as
   the ticket bot.
2. **Base directory**: set to `/vps-dashboard` (not `/`) — this is what
   tells Coolify to build from this subfolder instead of the ticket bot's.
