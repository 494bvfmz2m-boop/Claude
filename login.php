...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (!xs_csrf_verify($_POST['csrf_token'] ?? null)) {
        ...
        if ($action === 'login') {
            ...
            if (XyphrosAuth::isLoginThrottled($email)) {
                $error = 'Too many failed attempts on this account. Please wait 15 minutes and try again.';
                $u = null;
            } else {
                $u = XyphrosAuth::findByEmail($email);
            }

            if (!$error && (!$u || !XyphrosAuth::verifyPassword($pw, $u['password_hash']))) {
                XyphrosAuth::registerFailedLogin($email);
                $error = 'Incorrect email or password.';
            } elseif (!$error) {
                XyphrosAuth::clearFailedLogins($email);
            }

            if ($error) {
                // Already set above by the throttle check or a bad password.
